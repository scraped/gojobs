...
async function uploadJobs() {
  const jobs = await JobRaw.find().limit(10);
  ...
  uploadedJobs.forEach(job => {
    if (job instanceof Error) {
      console.log('Error uploading job:', job);
      errors++;
    }
  });
...
async function uploadJob(job) {
  ...
  if (crewUrl) {
    ...
    if (crew) {
      ...
      const crewInfo = await fetchCrewInfo(crewUrl);

      const { crewId, name, avatar } = crewInfo,
...
async function fetchCrewInfo(crewUrl) {
  ...
  if (!result) {
    throw new Error(`Cannot fetch crew info: ${crewUrl}`);
  }

  const response = result.data;

  const idAndAvatarMatch = response.match(/crews\/sc\/(\d+)\/(\d+)\/publish\//),
...
function getPlatform(platform) {
  const platformId = 1 + _.findIndex(platforms, p => p.name === platform);
  return platformId;
}
