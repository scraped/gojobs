...
JobRaw.find((err, jobs) => {
    ...
    jobs.forEach(job => {
      ...
      // We don't need jobs played < 10 times or by < 3 people
      // or that have < 2 likes. Survivals' rating is broken
      ...
      let ratingPoints = (likes + bkmk)
        ...
        * Math.max(0.82, (1 - likes / playTot))
        // + ((playTot / playUnq + playTot / likes) * 10)
        * (rating / 100)
        * (1 - (rating - ratingQuit) / 100);
        // * (ratingQuit / 100);

      // If the name starts with shit (or non-english letters), decrease rating
      if (name[0].match(/[^\w]/)) ratingPoints *= 0.1;

      // How many CAPITALIZED LETTERS are in the name, but min multiplier is 0.5
      ...
      // Also we don't need very bad jobs
