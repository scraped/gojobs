...
class RgscFetcher {
  ...
  redisKey() {
    return `rgsc-fetcher:${this.platform}`;
  }
  ...
  async getNewCredentials() {
    ...
    const request = await this.fetcher('/games/gtav/pc/playlists');

    console.log(request);

    const setCookieHttpHeader = request.headers['set-cookie'];
    ...
    const requestVerificationTokenMatch = request.data.match(REQUEST_VERIF_TOKEN_REGEX);
