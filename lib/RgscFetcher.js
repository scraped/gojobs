...
class RgscFetcher {
  ...
  static async setupRgscHttpClient(platform = 'pc') {
    const client = new RgscFetcher(platform);
    await client.getCredentials();
    return client;
  }

  static async fetchCrewPage(crewSlug) {
    return await axios({
      baseURL: 'https://socialclub.rockstargames.com/',
      url: `/crew/${crewSlug}`,
    });
  ...
  async getNewCredentials() {
    ...
    this.requestVerificationToken = requestVerificationTokenMatch[1];
