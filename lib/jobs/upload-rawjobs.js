...
exports.uploadRawJobs = async function({ limit = 5000 }) {
  ...
  console.log(`UPLOADING ${toUpload} JOBS`);
  ...
  uploadedJobs.forEach(job => {
    if (job instanceof Error) {
      console.log(`Failed to upload job:`, job);
      uploaded--;
    }
  });
...
async function uploadRawJob({ rawJob }) {
  ...
  if (jobOld) {
    job = _.assign(jobOld, job);
  }

  if (jobDetailsOld) {
    jobDetails = _.assign(jobDetailsOld, jobDetails);
  }

  await new Job(job).save();

  await new JobDetails(jobDetails).save();
...
function genExtendedJobObj({ jobId, rawJob }) {
  ...
  // for races
  if (rawJob.Metadata.data.mission.race) {
    const { chp, lap, rdis } = rawJob.Metadata.data.mission.race;
    specific.race = {
      chp,
      dist: rdis
    }
