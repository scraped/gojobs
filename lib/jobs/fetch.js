...
const {setupRgscHttpClient} = require('../RgscFetcher');
...
async function fetchJobBunch({
  ...
  const http = await setupRgscHttpClient(plat);

  const searchParams = genFetchJobBunchesData({category, id, offset});

  // We distinguish between platforms solely by cookies
  const jobBunch = (await http.fetcher({
