...
async function fetchBunches(options) {
  const { reqLimit, total: oldTotal, skip: oldSkip, lastFetch } = options;
...
async function fetchJobObject({ fetchResult, axiosOptions }) {
  const { jobId, jobCurrId } = fetchResult;
  ...
  if (!jobObject || !jobObject.MissionId) {
    ...
    console.log(`${jobId} fetch warn: the object is empty or broken`);
    return;
  }
  ...
  if (!latestVersion) {
    ...
    console.log(`${jobId} fetch warn: not the latest version, attempt to refetch`);
    return;
  }
  ...
  if (!jobObject.Content.stats && category === 'none') {
    ...
    console.log(`${jobId} fetch warn: doesn't have "stats" property`);
    return;
  }

  fetchResult.jobId = RootContentId;
  fetchResult.job = jobObject;
  console.log(`${jobId} fetched successfully`);
}
