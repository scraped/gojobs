...
async function fetchJobBunch({
  ...
  if (error || !jobs || !jobs.length) {
    ...
    throw new Error(`Error: current bunch of jobs is either broken or reporting the following: ${errorMessage}`);
  }

  return jobBunch;
}

async function fetchJobBunches2({
  category = 'all',
  platform = 'all',
  id,
  limit
