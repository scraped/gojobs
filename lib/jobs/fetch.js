...
exports.fetchAndSave = async function(options) {
  ...
  const { jobs, amount, fetched } = await fetchRgscJobs(options);
...
async function saveRgscJob(job) {
  ...
  console.log(`Job ${jobId}: saved as a raw job`);
}
...
async function fetchRgscJobs(options) {
  ...
  const iterations = Math.ceil(Math.min(limit, amount) / JOBS_PER_RESPONSE);
  ...
  responses.forEach(response => {
    if (response instanceof Error) {
      console.log(`Failed to fetch jobs from RGSC site: ${response.message}`);
    } else {
