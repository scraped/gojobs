...
async function fetchAllRgscJobs(options, kueJob) {
  ...
  let requests = [],
    responses = [],
    jobsIds = jobsFirstBunch.data.Missions.map(mission => mission.Mission)
      || [],
    jobs = [];
  ...
  responses.forEach(response => {
    ...
    jobsIds.push(...response.data.Missions.map(job => job.MissionId));
  });
  ...
  for (let i = 0; i < jobsIds.length; i++) {
    axiosOptions.params = { missionid: jobsIds[i] };
    requests.push(
      axios(axiosOptions).catch(err => err)
    )
  }
