...
function genFetchJobBunchesQuery({type, id, offset = 0}) {
  let query = {
    onlyCount: 'false',
...
async function fetchJobBunch({plat = 'pc', type = 'rockstar', id, offset = 0}) {
  ...
  if (jobBunch.Error || !jobBunch.Missions) {
    throw new Error(
      `Error, got the following response: ${JSON.stringify(
        jobBunch,
      )} for this search params: ${JSON.stringify(searchParams)}`,
    );
  }
