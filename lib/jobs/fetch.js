...
async function fetchAllRgscJobs(options, kueJob) {
  ...
  const jobsNumber = Math.min(limit, jobsFirstBunch.data.Total),
    iterations = Math.ceil(jobsNumber / JOBS_PER_RESPONSE);

  let jobsIds = Array.isArray(jobsFirstBunch.data.Missions)
    ? jobsFirstBunch.data.Missions.map(mission => mission.MissionId)
    : [];
  ...
  responses.forEach(response => {
    ...
    jobsIds.push(...response.data.Missions.map(mission => mission.MissionId));
  });
