...
function genExtendedJobObj({ jobId, rawJob }) {
  ...
  const bgFg = getBackgroundAndForeground({ rawJob });
...
function getBackgroundAndForeground({ rawJob }) {
  const { thumbnail } = rawJob.Metadata;

  jimp.read(thumbnail, (err, image) => {
    if (err) {
      console.log(err);
      return;
    }
    ...
    const colors = palette(pixels, 3);

    let foregroundLight = 0;
    ...
    return {
      background: colors,
      foregroundLight: foregroundLight >= 2
    };
  });
}
