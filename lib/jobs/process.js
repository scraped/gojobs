...
async function processAllJobs() {
  ...
  const jobsToProcessPromises = rawJobs.map((rawJob, i) => {
    let oldJob = previouslyUploaded[i];
    ...
    const differ = !oldJob
      || oldJob && oldJob.ver !== rawJob.Metadata.ver;
...
async function updateJobStats({ rawJob, oldJob }) {
  ...
  try {
    await new Job({ ...oldJob, stats }).save();
...
async function processJob({ rawJob, oldJob }) {
  ...
  try {
    ...
    await new Job({ ...oldJob, ...jobBasic }).save();
    ...
    console.log('processJob error:', error);
    return false;
  }
}
...
async function genExtendedJobObj({ rawJob }) {
  ...
  if (raceInfo) {
    ...
    chl.forEach(location => {
      chpLocs.push(location.x, location.y);
    });
    ...
    if (sndchk && secCheckpointsExist(sndchk)) {
      ...
      sndchk.forEach(location => {
        chpSecLocs.push(location.x, location.y);
      });
