...
async function processJobs(any = false) {
  const LIMIT = 300;
  const DAY = 60 * 60 * 24;
...
async function processJob({ rawJob, oldJob }) {
  ...
  try {
    let jobBasic = genBasicJobObj({ rawJob, oldJob });
...
function genBasicJobObj({ rawJob, oldJob }) {
  ...
  const stats = generateStats({ rgscJob, oldJob });
