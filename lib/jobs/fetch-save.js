...
async function fetchJobsAndSave(options) {
  ...
  console.log(chalk.green(`The following jobs fetched and saved: ${JSON.stringify(params)}`));

  if (fetchResults.length && category !== 'job' && !bunches && actualReqLimit === REQUESTS_LIMIT) {
...
async function fetchBunchesParams(options, fetchResults) {
  ...
  if (!fetchResults) {
    ...
    const { lastFetch, skip } = statistics;
    return { lastFetch, skip };
  }
