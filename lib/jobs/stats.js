...
function generateStats({ rawJob, oldJob }) {
  ...
  return {
    ...
    rating: ratingPct,
    growRate: growthRate({ rawJob, oldJob })
  };
}
...
function growthRate({ rawJob, oldJob }) {
  ...
  console.log(`growthPerDate: ${growthPerDate}`);

  const growth = (playedTotalNew / (playedTotalNew - growthPerDate)).toFixed(5);
