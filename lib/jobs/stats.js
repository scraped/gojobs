 ...
 */
function generateStats({ rgscJob, oldJob }) {
  ...
  return {
    ...
    rating: ratingPct,
    growRate: growthRate({ rgscJob, oldJob })
  };
}

function growthRate({ rgscJob, oldJob }) {
  if (!oldJob) {
    return 1;
  }

  const daysPassed = new Date() - oldJob.

  return rgscJob.stats.pt -
}
