...
function secondaryVehicles(raceInfo) {
  ...
  const checkpointsTransformVehicles = [...cptfrm, ...cptfrms];
...
function getScTypeAndMode({rgscJob}) {
  ...
  Object.keys(jobTypes).forEach(currType => {
    if (jobTypes[currType].name === scTypeName) {
      scTypeAndMode.scType = currType;
    }
  });
  ...
  if (modes) {
    Object.keys(modes).forEach(currMode => {
      if (modes[currMode].name === scModeName) {
        scTypeAndMode.scMode = currMode;
      }
    });
  }
...
function pitlaneAvailability({rgscJob}) {
  ...
  const {weap: weapons} = rgscJob.Metadata.data.mission;
