...
function calcGrowthRate({rawJob, oldJob}) {
  ...
  const growthRate = 1 + (actualGrowthRate - 1) * 3;
...
function checkLikes(likes, isSurvival) {
  const maxLikesOk = 50000;

  const lotOfLikesCoeff = isSurvival
    ? 1
    : Math.min(1, likes / maxLikesOk);
...
function checkVersionNumber(rgscJob, rockstarJob) {
  const maxVersions = 200;

  const version = rgscJob.Metadata.ver;

  const coeff = rockstarJob
    ? 0
    : Math.min(1, version / maxVersions);
...
function checkProps(rgscJob) {
  ...
  const {prop: props} = rgscJob.Metadata.data.mission;
