...
async function fetchCrew({slug}) {
  ...
  // #rrggbbaa -> rrggbb
  const crewColor = data.crewColor.substring(1, 7);
  ...
  const avatarElementSelector = '.CrewHeader__emblemWrap__2QHFY > span:nth-child(1) > img:nth-child(1)';

  const browser = await puppeteer.launch();
  const page = await browser.newPage();
  await page.goto(crewUrl);

  const avatarElement = page.$(avatarElementSelector);
