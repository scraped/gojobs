...
const {setupBasicRgscFetcher} = require('./RgscFetcher');
...
async function fetchAndSaveCrewInfo({slug}) {
  ...
  const fetcher = setupBasicRgscFetcher();
  ...
  if (nonRockstar) {
    ...
    const platformsToAdd = platformsIds.filter(
      currPlatform =>
        !find(crewFetchInfo, fetchInfo => fetchInfo.plat === currPlatform),
    );

    await Promise.all(
      platformsToAdd.forEach(currPlatform =>
        new FetchInfo({
          type: 'crew',
          id: slug,
          plat: currPlatform,
